...
func (c *OrderClient) AddOrderImage(orderID int32, imageURL string) (*orderpb.AddOrderImageResponse, error) {

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	return c.client.AddOrderImage(ctx, &orderpb.AddOrderImageRequest{
		OrderId:  orderID,
		ImageUrl: imageURL,
	})
}

func (c *OrderClient) GetOrderImage(orderID int32) (*orderpb.GetOrderImageResponse, error) {

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	return c.client.GetOrderImage(ctx, &orderpb.GetOrderImageRequest{
		OrderId: orderID,
	})
}
...
func (c *OrderClient) GetTopRestaurantsByOrders(limit int) (*orderpb.GetTopRestaurantsByOrdersResponse, error) {

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	return c.client.GetTopRestaurantsByOrders(ctx, &orderpb.GetTopRestaurantsByOrdersRequest{
		Limit: int32(limit),
	})
}
